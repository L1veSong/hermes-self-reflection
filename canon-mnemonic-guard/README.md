...
**Layer 3 — sentinel Plugin**：内核级输出拦截 + 步骤完整性检查。v1.3.0(17hooks+pre_tool_call+自披露闭环)——直接 patch SKILL.md 被内核拦截、断言须附证据、步骤没做完禁止 LLM 回复。
...
用户消息
  ...
  ├─ 步骤完整性检查（v5.5.5）: 链接/文件/clarify/workflow 四重强制验证
...
sentinel/      v1.3.0   Plugin — 17hooks硬拦截 + pre_tool_call阻断 + 自披露闭环
```
